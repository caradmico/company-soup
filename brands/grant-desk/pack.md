# Grant Desk

Lane: Professional services · Graphic Oregon child · grants

## One-liner
Grant writing for nonprofits, arts orgs, and community organizations — quote only, package prep for the authorized submitter.

## Offer
Quote-only grant-writing service (Cara Mico / Company Soup child under Graphic Oregon).

Deliverables:
- Funder fit scan + go/no-go
- Narrative draft
- Budget narrative / attachments checklist
- Submission package prep for the client's authorized submitter

Proof (only these): MA in arts administration & nonprofit management; ~$1M in grants funded.

Hard gate: she does not submit applications without Cara's explicit gate. CTA is quote/intake only.

## Path
- Brand door: https://caradmico.github.io/company-soup/brands/grant-desk/
- Free resources: https://caradmico.github.io/company-soup/brands/grant-desk/resources/
- CTA: mailto hello@graphicoregon.com — subject “Grant Desk — scope quote”
- Parent: Graphic Oregon

## Done-when
Stranger can open the door, see why / who it's for / what she delivers / proof, open free grant resources, and request a quote without login.

## Do-not
Invent RFPs, funders, client wins, or dollar amounts beyond ~$1M funded · claim auto-submit · rewrite Demeter / StarIS / watershed / Soup Ops · Three.js / Printify / Intern commodity PDF
